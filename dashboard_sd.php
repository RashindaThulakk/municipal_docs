...
if (!empty($_GET['edit_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM sd WHERE id=? LIMIT 1");
    $stmt->execute([$id]);
    $edit = $stmt->fetch();
}
...
$sql = "SELECT id, sd_no, year, rack_number, applicant_name, applicant_id, assessment_ward, created_at
        FROM sd
        WHERE sd_no LIKE ? AND CAST(year AS CHAR) LIKE ? AND applicant_id LIKE ?
        ORDER BY created_at DESC
        LIMIT 200";
$stmt = $pdo->prepare($sql);
$stmt->execute([$sd_search, $year_search, $app_search]);
$rows = $stmt->fetchAll();
?>
...
<div class="row">
  ...
  <div class="col-lg-4 col-md-5">
    <div class="left-panel">
      ...
      <form method="post" action="add_sd.php" enctype="multipart/form-data" onsubmit="return validateForm()">
        ...
        <div class="mb-2">
          <label class="form-label">SD No</label>
          <input id="sd_no" name="sd_no" class="form-control" value="<?php echo $edit ? htmlspecialchars($edit['sd_no']) : ''; ?>" required>
        </div>
        ...
        <div class="mb-2">
          ...
          <div id="filePreview" class="mt-2">
            <?php if ($edit && $edit['attachment_type']): ?>
              ...
              <a href="download_attachment_sd.php?id=<?php echo $edit['id']; ?>" target="_blank">Existing attachment — view</a>
            <?php endif; ?>
          </div>
        </div>
        ...
        <div class="d-grid gap-2">
          ...
          <?php if ($edit): ?>
            <a href="dashboard_sd.php" class="btn btn-secondary">Cancel Edit</a>
          <?php endif; ?>
        </div>
      </form>
    </div>
  ...
  <div class="col-lg-8 col-md-7">
    <div class="right-panel">
      <div class="row mb-3">
        <div class="col-md-4">
          <input class="form-control" id="s_sd_no" placeholder="Search SD No (partial allowed)" value="<?php echo htmlspecialchars($sd_no); ?>" onkeyup="searchSuggest()">
        </div>
        <div class="col-md-3">
      ...
      <div class="table-wrap">
        <table class="table table-sm table-hover">
          <thead class="table-light">
            <tr>
              <th>SD No</th><th>Year</th><th>Rack</th><th>Applicant</th><th>ID</th><th>Ward</th><th>Attachment</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            <?php if ($rows): foreach ($rows as $r): ?>
              <tr>
                <td><?php echo htmlspecialchars($r['sd_no']); ?></td>
                ...
                <td><a href="download_attachment_sd.php?id=<?php echo $r['id']; ?>" target="_blank">View</a></td>
                <td>
                  <a href="dashboard_sd.php?edit_id=<?php echo $r['id']; ?>" class="btn btn-sm btn-primary">Edit</a>
                  <?php if ($_SESSION['role'] === 'Admin' || $_SESSION['role'] === 'Clerk'): ?>
                    <a href="delete_record_sd.php?id=<?php echo $r['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this record?')">Delete</a>
                  <?php endif; ?>
                </td>
              </tr>
            <?php endforeach; else: ?>
